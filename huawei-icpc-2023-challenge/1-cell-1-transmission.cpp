...
int one_cell_one_transmission() {
    ...
    for (int j = 0; j < J; j++) {
        ...
        int l = frame[j][4];
        long double g = 0;
        vector<pair<pair<long double, int>, pair<int, int>>> candidates;
        for (int t = t_0; t < t_0 + l; t++) {
            if (used[t]) continue;
            for (int k = 0; k < K; k++) {
                for (int r = 0; r < R; r++) {
                    candidates.push_back({{s_0[t][k][r][n], t}, {k, r}});
                }
            }
        }
        ...
        for (int i = 0; i < candidates.size() && tbs > g; i++) {
            int t = candidates[i].first.second;
            int k = candidates[i].second.first;
...
int main() {
    ...
    int score = one_cell_one_transmission();
